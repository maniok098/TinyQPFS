...
for k = 1:length(time)-N-1
  ...
  % read control action
  vm_des = solutionOL(idx_first_ctrl);
...
u_MPC_1 = u_sim_nMPC;
%% nominal MPC with only equality constraints, solved using KKT condition
...
for k = 1:length(time)-N-1
  ...
  % construct KKT and solve pinv only at setup
  if k==1
      KKT = [H, Aeq';
             Aeq, zeros(size(Aeq,1))];
      KKT_pinv = pinv(KKT);
      KKT_pinv_ctrl = KKT_pinv(idx_first_ctrl,:);
  end
  tic
  % calculate control action
  vm_des = KKT_pinv_ctrl*rhs;
  runtime = runtime +toc;
